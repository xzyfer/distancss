var walk = require('./walk.js'),
    ...
    re = {
        atRules: "@-(?:moz|webkit|o)-|(?:keyframes|charset|import|media|page|font-face|namespace)"
    },
    sectionSemver = '0.0.0';
...
parse = function(input, options, callback) {
    ...
    data.sections = dedupeSections(data.sections);

    data.section_refs = {};
    data.sections.forEach(function(section) {
        data.section_refs[section.data.reference] = section;
    });
...
/**
 * Returns an array of sections deduped but title
 * @param  {Array} sections   The current array of sections
 * @return {Array} An array of sections deduped by title.
 */
dedupeSections = function(sections) {
    var titles = [],
        re = new RegExp('[.]'),
        major = 0,
        blockMajor = false,
    ...
    return sections.filter(function(section) {
        ...
        if(!re.test(section.reference())) {
            major = section.reference();

            if(titles.indexOf(section.header()) > -1) {
                blockMajor = true;
                return false;
            } else {
                titles.push(section.header());
                return true;
            }
        } else {
            reMajor = new RegExp('^'+section.reference());
            if(blockMajor && reMajor.test(section.reference())) {
                return false;
            } else {
                return true;
            }
...
checkReference = function(sectionData, options) {
    ...
    if((tag = getTag(sectionData, 'section'))) {
        // is semver?
        if((match = tag.value().trim().match(/^([0-9\.]+)/i))) {
            reference = match[1];
        } else
        // is it a bump string
        if((match = tag.value().trim().match(/^#next-(reset|major|minor|patch)/i))) {
            if(match[1] === "reset") {
                sectionSemver = '1.0.0';
            } else {
                sectionSemver = semver.inc(sectionSemver, match[1]);
            }
            reference = sectionSemver;
        }
    }
...
processTags = function(sectionData, options) {
    sectionData.unprocessed = sectionData.unprocessed.map(function(line, index) {
        if (!hasPrefix(line, options, '@Markup') && !hasPrefix(line, options, re.atRules) && hasPrefix(line, options, '@')) {
