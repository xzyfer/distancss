...
suite('Section', function() {
	common.hasMethod(new Section({}), 'header');
	common.hasMethod(new Section({}), 'description');
	common.hasMethod(new Section({}), 'deprecated');
	common.hasMethod(new Section({}), 'experimental');
	common.hasMethod(new Section({}), 'modifiers');
	common.hasMethod(new Section({}), 'firstModifier');
	...
	suite('#modifiers', function() {
		...
		common.testAllSections('() all returned should be instances of Modifier', '*.less|*.css', function(section) {
			var modifiers = section.modifiers(),
				i, l = modifiers.length;

			for (i = 0; i < l; i += 1) {
				assert.ok(modifiers[i] instanceof Modifier);
			}
		});
